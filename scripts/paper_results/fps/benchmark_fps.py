...
def benchmark_environment(env_name: str, num_envs: int, algorithm: str = "happo", num_steps: int = 500) -> float:
    """
    ...
    try:
        ...
        cmd = [
            ...
            env_name,
            "--save_interval",
            "1000000",  # Don't save during benchmark
            "--log_interval",
            "1",
            "--headless",
        ]
        ...
        try:
            ...
            try:
                ...
                print(" ERROR: Failed to capture output after timeout")
            return None
        ...
        if last_fps is not None:
            ...
            print(" ERROR: No FPS found in output")
            return None
...
def create_plots(results: list, output_dir: Path):
    ...
    env_names = sorted({r["environment"] for r in results})
    num_envs_list = sorted({r["num_envs"] for r in results})
    ...
    ax.set_xticklabels(
        [name[:15] + "..." if len(name) > 15 else name for name in env_names], rotation=45, ha="right", fontsize=9
    )
        ...
        for j in range(len(num_envs_list)):
            ...
            if fps_val > 0:
                ax.text(
                    ...
                    ha="center",
                    va="center",
                    color="black" if fps_val < fps_matrix.max() / 2 else "white",
                    fontsize=8,
                )
