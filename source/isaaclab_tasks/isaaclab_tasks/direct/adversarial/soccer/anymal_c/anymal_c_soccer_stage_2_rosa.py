...
@configclass
class AnymalStage2SoccerRosaEnvCfg(DirectMARLEnvCfg):
    decimation = 4
    ...
    state_space = 4
    state_spaces = {f"robot_{i}": 4 for i in range(1)}
    ...
    reward_scales = {
        "dist_to_ball_reward": [0.0, 0.5, 5.0],
        "ball_velocity_reward": [0.0, 0.02, 0.001],
        "ball_to_goal_reward": [1.0, 2.0, 2.0],
        "goal_reward": [20.0, 20.0, 20.0],
    }


class AnymalStage2SoccerRosaEnv(DirectMARLEnv):
    cfg: AnymalStage2SoccerRosaEnvCfg

    def __init__(
        self, cfg: AnymalStage2SoccerRosaEnvCfg, render_mode: str | None = None, headless: bool | None = None, **kwargs
    ):
        ...
        self.reward_scale_tables = self._build_reward_scale_tables()
        self.num_reward_scale_profiles = next(iter(self.reward_scale_tables.values())).shape[0]
        self.reward_scale_index = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
    ...
    def _build_reward_scale_tables(self) -> dict[str, torch.Tensor]:
        reward_scale_tables = {
            reward_name: torch.tensor(reward_values, dtype=torch.float32, device=self.device)
            for reward_name, reward_values in self.cfg.reward_scales.items()
        }

        profile_lengths = {reward_name: reward_values.shape[0] for reward_name, reward_values in reward_scale_tables.items()}
        if len(set(profile_lengths.values())) != 1:
            raise ValueError(f"All reward scale profiles must have the same length, got: {profile_lengths}")

        return reward_scale_tables

    def _get_active_reward_scales(self) -> dict[str, torch.Tensor]:
        return {
            reward_name: reward_values[self.reward_scale_index]
            for reward_name, reward_values in self.reward_scale_tables.items()
        }

    def _get_active_reward_scale_tensor(self) -> torch.Tensor:
        # Package the active reward profile per environment for the critic/state path.
        return torch.stack(
            [reward_values[self.reward_scale_index] for reward_values in self.reward_scale_tables.values()],
            dim=-1,
        )
    ...
    def _get_states(self) -> torch.Tensor:
        states = self._get_active_reward_scale_tensor()
        return torch.nan_to_num(states, nan=0.0, posinf=1e6, neginf=-1e6)

    def _get_rewards(self) -> dict:
        active_reward_scales = self._get_active_reward_scales()
        ...
        ball_vel_reward = torch.linalg.norm(self.ball.data.root_vel_w[:, :3], dim=1)
        ...
        rewards = {
            "dist_to_ball_reward": robot_distance_to_ball_mapped * active_reward_scales["dist_to_ball_reward"] * self.step_dt,
            "ball_velocity_reward": ball_vel_reward * active_reward_scales["ball_velocity_reward"] * self.step_dt,
            "ball_to_goal_reward": ball_distance_to_goal_mapped * active_reward_scales["ball_to_goal_reward"] * self.step_dt,
            "goal_reward": goal_reward * active_reward_scales["goal_reward"],
        }
        ...
        self.reward_scales = self._get_active_reward_scale_tensor()
    ...
    def _get_dones(self) -> tuple[dict, dict]:
        ...
        fallen = self.robots["robot_0"].data.root_com_pos_w[:, 2] < 0.1
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self.reward_scale_index[env_ids] = torch.randint(
            0, self.num_reward_scale_profiles, (len(env_ids),), device=self.device
        )
        ...
        goals_scored = (
            ((ball_in_goal1[env_ids] & (target_goals == 0)) | (ball_in_goal2[env_ids] & (target_goals == 1)))
            .to(torch.float32)
            .sum()
            .item()
        )
