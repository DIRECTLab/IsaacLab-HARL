...
@configclass
class AnymalVsLeatherbackSoccerEnvCfg(DirectMARLEnvCfg):
    decimation = 4
    episode_length_s = 30.0
    action_scale = 0.5
    anymal_action_spaces = {f"anymal_{i}": 12 for i in range(1)}
    leatherback_action_spaces = {f"leatherback_{i}": 2 for i in range(1)}
    action_spaces = anymal_action_spaces | leatherback_action_spaces
    anymal_observation_spaces = {f"anymal_{i}": 66 for i in range(1)}
    leatherback_observation_spaces = {f"leatherback_{i}": 24 for i in range(1)}
    ...
    possible_agents = ["anymal_0", "leatherback_0"]

    teams = {
        "team_0": ["anymal_0"],
        "team_1": ["leatherback_0"],
    }
    ...
    anymal_0.init_state.pos = (0, 0, 0.5)
...
class AnymalVsLeatherbackSoccerEnv(DirectMARLEnv):
    cfg: AnymalVsLeatherbackSoccerEnvCfg
