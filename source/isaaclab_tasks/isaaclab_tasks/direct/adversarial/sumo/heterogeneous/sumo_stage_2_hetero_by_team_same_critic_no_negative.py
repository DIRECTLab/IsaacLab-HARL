...
@configclass
class SumoStage2HeteroByTeamSameCriticNoNegativeEnvCfg(DirectMARLEnvCfg):
    decimation = 4
    ...
    # Teams
    teams = {
        "team_0": ["robot_0", "robot_1", "robot_2", "robot_3"],
    }
    adversarial_teams = {
    ...
    robot_1: ArticulationCfg = ANYMAL_C_CFG.replace(prim_path="/World/envs/env_.*/Robot_2")
    robot_1.init_state.rot = get_quaternion_tuple_from_xyz(0, 0, torch.pi / 2)
    robot_1.init_state.pos = (-1.0, 0.0, 0.3)

    robot_2: ArticulationCfg = LEATHERBACK_CFG.replace(prim_path="/World/envs/env_.*/Robot_1")
    robot_2.init_state.rot = get_quaternion_tuple_from_xyz(0, 0, torch.pi / 2)
    robot_2.init_state.pos = (0.0, -1.0, 0.1)
    ...
    robot_3.init_state.pos = (1.0, 0.0, 0.1)
...
class SumoStage2HeteroByTeamSameCriticNoNegativeEnv(DirectMARLEnv):
    cfg: SumoStage2HeteroByTeamSameCriticNoNegativeEnvCfg
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.debug = debug
        self._critic_team = next(iter(self.cfg.teams))
        self._adversarial_teams = copy.deepcopy(self.cfg.adversarial_teams)
        self._agent_to_adversarial_team = {
            robot_id: team for team, robots in self._adversarial_teams.items() for robot_id in robots
        }

        self._throttle_dof_idx, _ = self.robots["robot_2"].find_joints(self.cfg.throttle_dof_name)
        self._steering_dof_idx, _ = self.robots["robot_2"].find_joints(self.cfg.steering_dof_name)
    ...
    def _setup_scene(self):
        ...
        self.animals = {"robot_0": self.robots["robot_0"], "robot_1": self.robots["robot_1"]}
        self.leatherbacks = {"robot_2": self.robots["robot_2"], "robot_3": self.robots["robot_3"]}
    ...
    def _pre_physics_step(self, actions: dict) -> None:
        ...
        for robot_id in self.leatherbacks.keys():
            ...
            self._steering_action = (
                actions[robot_id][:, 1].repeat_interleave(2).reshape((-1, 2)) * self.cfg.steering_scale
            )
    ...
    def _get_observations(self) -> dict:
        ...
        obs[self._critic_team] = {}
        for robot_id in self.cfg.teams[self._critic_team]:
            team = self._agent_to_adversarial_team[robot_id]
            teammate_id = [agent for agent in self._adversarial_teams[team] if agent != robot_id][0]
            enemy_team = "team_1" if team == "team_0" else "team_0"
            enemies = self._adversarial_teams[enemy_team]
            ...
            obs[self._critic_team][robot_id] = robot_obs
    ...
    def _get_rewards(self) -> dict:
        ...
        for team, agents in self._adversarial_teams.items():
        ...
        push_out_reward_0 = tie * (team1_lost - time_out) * self.cfg.reward_scale
        push_out_reward_1 = tie * (team0_lost - time_out) * self.cfg.reward_scale
        total_reward = push_out_reward_0 + push_out_reward_1
    ...
    def _get_dones(self) -> tuple[dict, dict]:
        ...
        dones = {self._critic_team: done}

        time_out = self.episode_length_buf >= self.max_episode_length - 1
        timeouts = {self._critic_team: time_out}
