...
@configclass
class AnymalSoccerMAHomoEnvCfg(DirectMARLEnvCfg):
    decimation = 4
    episode_length_s = 30.0
    action_scale = 0.5
    action_spaces = {f"robot_{i}": 12 for i in range(4)}
    observation_spaces = {f"robot_{i}": 66 for i in range(4)}
    state_space = 0
    ...
    # robot_0.init_state.rot = get_quaternion_tuple_from_xyz(0,0,torch.pi)
    robot_0.init_state.pos = (-1.0, -2.0, 0.3)
    robot_1: ArticulationCfg = ANYMAL_C_CFG.replace(prim_path="/World/envs/env_.*/Robot_1")
    # robot_1.init_state.rot = get_quaternion_tuple_from_xyz(0,0,torch.pi)
    robot_1.init_state.pos = (-1.0, 2.0, 0.3)
    ...
    robot_2.init_state.pos = (1.0, -2.0, 0.3)
    ...
    robot_3.init_state.pos = (1.0, 2.0, 0.3)
...
class AnymalSoccerMAHomoEnv(DirectMARLEnv):
    cfg: AnymalSoccerMAHomoEnvCfg

    def __init__(
        self, cfg: AnymalSoccerMAHomoEnvCfg, render_mode: str | None = None, headless: bool | None = None, **kwargs
    ):
        ...
        self.penalty_box = {
            agent: torch.zeros(self.num_envs, 1, dtype=torch.bool, device=self.device) for agent in self.robots.keys()
        }
        self.penalty_timer = {agent: torch.zeros(self.num_envs, 1, device=self.device) for agent in self.robots.keys()}
    ...
    def _get_rewards(self) -> dict:
        ...
        team_0_loss_reward = -ball_in_goal1.to(torch.float32) - out_of_arena.to(
            torch.float32
        )  # - fallen_team_0.to(torch.float32)
        team_1_score_reward = ball_in_goal1.to(torch.float32) * self.cfg.goal_reward_scale
        team_1_loss_reward = -ball_in_goal2.to(torch.float32) - out_of_arena.to(
            torch.float32
        )  # - fallen_team_1.to(torch.float32)
    ...
    def _get_fallen_robots(self, robot_ids) -> dict[str, torch.Tensor]:
        fallen = {}
        for robot_id in robot_ids:
            fallen[robot_id] = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
            fallen[robot_id] = self.robots[robot_id].data.root_com_pos_w[:, 2] < 0.09
        return fallen
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        ball_default_state[:, :2] = (
            ball_default_state[:, :2] + self.scene.env_origins[env_ids][:, :2] + sampled_grid_pos[:, self.num_robots]
        )
    ...
    def _sample_positions_grid(self, env_ids, num_samples, min_dist=1.0, grid_spacing=1.0):
        ...
        for i in range(N):
            xs = torch.arange(env_origins[i, 0] - 8, env_origins[i, 0] + 9, grid_spacing, device=device)
            ys = torch.arange(env_origins[i, 1] - 3, env_origins[i, 1] + 4, grid_spacing, device=device)
