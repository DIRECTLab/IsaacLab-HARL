...
@configclass
class LeatherbackVSLeatherbackSumoStage1EnvCfg(DirectMARLEnvCfg):
    decimation = 4
...
class LeatherbackVSLeatherbackSumoStage1Env(DirectMARLEnv):
    cfg: LeatherbackVSLeatherbackSumoStage1EnvCfg

    def __init__(
        self, cfg: LeatherbackVSLeatherbackSumoStage1EnvCfg, render_mode: str | None = None, headless: bool | None = None, **kwargs
    ):
    ...
    def _get_rewards(self) -> dict:
        ...
        # time penalty to encourage finishing faster
        time_penalty = self.cfg.time_penalty * torch.ones_like(dist_from_center, device=self.device)

        out = self._robots_out_of_ring()
        r0_lost = out["robot_0"].to(torch.float32)
        r1_lost = out["robot_1"].to(torch.float32)
        push_out_reward = (r1_lost - r0_lost) * self.cfg.reward_scale

        rewards = {
            "other_dist_from_center_reward": dist_from_center_mapped * self.step_dt,
            "dist_to_other_robot_reward": close_to_other_robot_mapped * self.step_dt,
            "time_out_reward": time_penalty,
            "push_out_reward": push_out_reward,
        }
        ...
        self._episode_sums["other_dist_from_center"] = dist_from_center
        self._episode_sums["dist_to_other_robot"] = close_to_other_robot

        return {
            "team_0": reward,
            "team_1": torch.zeros_like(reward, device=self.device),
        }
